...
open class StreamPeerTCP: StreamPeer {
    ...
    public func status() -> Godot.StreamPeerTCP.Status {
        var __temporary = Godot.StreamPeerTCP.Status.RawValue(0)
        withUnsafeMutablePointer(to: &__temporary) { (__ptr___temporary) in
            `self`.withUnsafeRawPointer { (__ptr_self) in
                gdextension_interface_object_method_bind_ptrcall(
        ...
        return Godot.StreamPeerTCP.Status(rawValue: __temporary)!
    }
    ...
    private static var __method_binding_disconnect_from_host: GDExtensionMethodBindPtr = {
        _$staticClassName.withUnsafeRawPointer { __ptr__class_name in
        GodotStringName(swiftStaticString: "disconnect_from_host").withUnsafeRawPointer { __ptr__method_name in
